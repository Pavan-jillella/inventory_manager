...
import { DetailDialog } from './Analytics';
import { UsageHeatmap } from './VisualCharts';
...
export function ProductDetails({ item, onClose }) {
  ...
  return <DetailDialog title={item.name} onClose={onClose}>
    ...
    <UsageHeatmap title="Daily usage calendar · last 30 days" data={Array.from({ length: 30 }, (_, index) => { const date = shiftDate(today, index - 29); return { date, used: issues.filter(row => dateKey(row.timestamp) === date).reduce((sum, row) => sum + row.quantity, 0) }; })} />
