...
function BreakfastTrends({ date, days, data, save }) {
  ...
  return <>
    ...
    {item && <TrendChart title={`${item.name} · weekly breakfast usage`} data={chart} bars series={[{ key: 'served', name: `Served (${item.unit})` }, { key: 'wasted', name: `Wasted (${item.unit})` }, { key: 'unclassified', name: `Unclassified (${item.unit})` }]} note="Zero means no quantity recorded for that action. Served-per-guest needs a guest count and a served entry." />}
  </>;
}
